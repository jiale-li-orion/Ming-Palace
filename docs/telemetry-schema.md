...
| `sessionId` | string? | UUID v4；内容初始化失败等会话前事件可为 null |
...
| Event | Payload | Description |
|-------|---------|-------------|
| `state_transition` | `{fromState, toState, eventType, route}` | 状态机完成一次合法转换 |
| `invalid_transition` | `{fromState, eventType, error}` | 非法转换被拒绝 |
...
| `audio_loaded` | `{asset: string}` | 恢复会话时仅加载但不自动播放 |
| `audio_replayed` | `{asset: string}` | 从头重播 |

### Interaction events

| Event | Payload | Description |
|-------|---------|-------------|
| `question_choice` | `{choice: string}` | User chose a branch: `feudal` or `classics` |
...
| Event | Payload | Description |
|-------|---------|-------------|
| `app_error` | `{code: string, detail?: string}` | 非致命错误；当前代码包含 `content_load_failed`、`audio_load_failed`、`image_load_failed` |
